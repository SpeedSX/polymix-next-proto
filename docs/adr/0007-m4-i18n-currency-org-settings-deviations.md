...
### 1. `default_language` is `"en" | "ua"`, not `"en" | "de"`
...
is explicit that the supported pair is `en` + `ua` ("`ua` + `en` (typical
print-market pair)... Make `ua` default choice") and M0–M3 already only
ever set `"en"`. Treated the schema block as the stale one and implemented
`en`/`ua` throughout — there is no `de` locale anywhere in the frontend.
...
### 3. The app's boot language stays `en`; `ua` is opt-in via the switcher

The Architecture section says to "make `ua` default choice". Doing that
literally — defaulting `i18next`'s `lng` to `ua` — would flip every
...
to `ua` works fully (full translation, reformatted dates/numbers) — it
doesn't require `ua` to be the initial render. Kept `en` as the actual
runtime default and boot language; `ua` is fully translated and reachable
...
`fake::faker::name::uk_UA::Name` to swap in. `crates/seeder/src/ua.rs` is a
...
1. `default_language` stays a free-form `String` in code (not a Rust enum);
   the two values actually produced/consumed are `"en"` and `"ua"`.
...
3. `frontend/src/lib/i18n/index.ts` keeps `lng: 'en'`/`fallbackLng: 'en'`;
   ...
   top, never changing the no-preference-stored default.
4. `crates/seeder/src/ua.rs` owns the Ukrainian name/address data; selected
   via `SEED_LOCALE=ua` (`just seed-ua`), which also switches the
   provisioned tenant's `default_language`/`default_currency` to
   `ua`/`UAH` via `TenantProvisioner::provision_with_locale`.
...
- A user who never touches the language switcher always sees English,
  regardless of their tenant's `default_language` — including on the `ua`
